...
// RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/trunk/OmniGroup/Frameworks/OmniAppKit/OATrackingLoop.m 170958 2012-08-03 22:04:57Z tom $");

#define DEBUG_LOOP(format, ...) do { \
    if (_debug) \
        NSLog(@"TRACKING IN %@: " format, [_view debugDescription], ## __VA_ARGS__); \
} while(0)
...
- init;
{
    [NSException raise:NSInternalInconsistencyException format:@"Unused method %s", __PRETTY_FUNCTION__];
    return nil;
}
...
- (NSSize)draggedOffsetInView:(NSView *)view;
{
    ...
    NSCAssert([view window] == [_view window], @"wrong window");
...
- _initWithView:(NSView *)view mouseDown:(NSEvent *)event;
{
    NSCAssert(view, @"required argument");
    NSCAssert(event, @"required argument");
    ...
    switch ([_mouseDownEvent type]) {
        ...
        case NSOtherMouseDown:
            // TODO: Need to record which mouse button went down and only signal up when *that* button goes up.
            break;
    }
    ...
    while (!_stopped) {
        @autoreleasepool {
            ...
            if (eventType == _upEventType ||
                ...
                NSCAssert(_shouldAutoscroll, nil);
                if ([_view autoscroll:lastMouseEvent]) {
                    ...
                    NSCAssert(lastMouseEvent != nil, nil);
...
{
    return [[OATrackingLoop alloc] _initWithView:self mouseDown:mouseDownEvent];
}
